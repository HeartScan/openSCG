...
def preprocess_web_signal(samples: List[Dict[str, float]]) -> Tuple[np.ndarray, np.ndarray]:
    """
    ...
    timestamps = np.array([float(point.get("timestamp", 0)) for point in samples])
    ...
    return resampled_signal, regular_timestamps + start_time_ms
